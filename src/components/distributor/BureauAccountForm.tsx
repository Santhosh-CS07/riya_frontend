...
const BureauAccountForm: React.FC = () => {
  ...
  const distributorId: any = localStorage.getItem('distributorId');
  ...
  const validateForm = () => {
    ...
    if (!formData.bureauName.trim()) newErrors.bureauName = 'Bureau Name is required';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res: any = await registerBureauAccount(formData_);
      if (res.status === 1) {
  ...
  return (
    <>
      <Navbar title={companyName} tab1="Home" tab2="12345" tab3="Profile" distributorId={distributorId} screenType="DISTRIBUTOR" />
      <div>
        {showNextComponent ? <>
          ...
          <div className="bg-white p-6 sm:p-8 rounded shadow-md w-full max-w-xs sm:max-w-md">
            <h1 className="text-3xl font-bold mb-6 text-center font-serif">Bureau Account Registration</h1>
            <div className="mb-4">
              ...
              />
              {errors.bureauName && <p className="text-red-500 text-xs mt-1">{errors.bureauName}</p>}
            </div>
            <div className="mb-4">
